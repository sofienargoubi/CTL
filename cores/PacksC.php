...
function AfficherPackbo($packs){
	...
	function ajouterPacks($packs,$image){
		$sql="insert into packs (nom,prix,informations,livres,image) values (:nom,:prix,:informations,:livres,:image)";
...
function recupererPacksParId($id){
	...
	function GetPacksParPrix(){
		$sql="select nom,count(*) as Number from packs group by nom order by prix";
		$db = config::getConnexion();
		try{
		$liste=$db->query($sql);
		return $liste;
