...
class CommandeC{
    ...
    function afficherCommandespecific($id){
        ...
        $sql="SElECT ID_Commande,Date_Creation,Etat_Commande From commande inner join users on ID=ID_User where ID_User=$id";
        $DB = config::getConnexion();
        try{
            $liste=$DB->query($sql);
            return $liste;
    ...
    function afficheruser($id){
        ...
        $sql="SElECT Nom From users u inner join commande c on u.ID=c.ID_User where ID_Commande=$id";
        $DB = config::getConnexion();
        $req=$DB->prepare($sql);
        try{
            $req->execute();
