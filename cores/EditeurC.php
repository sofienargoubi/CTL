...
class EditeurC {
    ...
    function rechercherEditeurTop(){
        $sql="select * from editeur order by nb_Vente desc limit 5";
        $db = config::getConnexion();
        try{
            $liste=$db->query($sql);
            return $liste;
	...
	function afficherEditeurrech($nom){
		//$sql="SElECT * From editeur e inner join formationphp.editeur a on e.cin= a.cin";
		$sql="SElECT * From editeur where nomMaison like '%$nom%'";
		$db = config::getConnexion();
		try{
		$liste=$db->query($sql);
		return $liste;
